...
def main(args):
    ...
    for i in range(0, metapath_length):
        if metapath[i] not in metapath_dict.keys():
            metapath_dict[metapath[i]] = index
            index += 1
    metapath_dict =  {k: v for k, v in sorted(metapath_dict.items(), key=lambda item: item[1])}
    meta = []
    for elm in metapath:
        meta.append(metapath_dict[elm])
    meta.reverse()
    print('metapath: ', meta)
    ...
    for i in range(0, num_nodes):
        ...
        links[i] = random.randint(5, args.max_node_degree)
    ...
    # Create a directory to save files
        path = '/Users/francescoferrini/VScode/MultirelationalGNN/data/deterministic/complex/length_m_' + str(metapath_length) + '__tot_rel_' + str(args.num_rel_types)
    else: 
