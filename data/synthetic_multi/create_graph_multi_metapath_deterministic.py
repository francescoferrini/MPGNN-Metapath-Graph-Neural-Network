...
def main(args):
    ...
    if args.metapath2:
        ...
        for elm in metapath2_split:
            if elm.isdigit(): # this is a relation
    ...
    for i in range(0, num_nodes):
        ...
        for j in range(0, num_links):
            ...
            if colors[src] == 0:
                ...
                rel.append(r)
                if r == 0 or r == 1:
                    dst = random.choice([chiave for chiave, valore in colors.items() if valore == 1])
                elif r == 2 or r == 3: 
                    dst = random.choice([chiave for chiave, valore in colors.items() if valore == 2])
            elif colors[src] == 1:
                r = random.choice([4, 5, 6, 7])
                rel.append(r)
                if r == 4 or r == 5:
                    dst = random.choice([chiave for chiave, valore in colors.items() if valore == 0])
                elif r == 6 or r == 7: 
                    dst = random.choice([chiave for chiave, valore in colors.items() if valore == 2])
            elif colors[src] == 2:
                r = random.choice([8, 9, 10, 11])
                rel.append(r)
                if r == 8 or r == 9:
                    dst = random.choice([chiave for chiave, valore in colors.items() if valore == 0])
                elif r == 10 or r == 11: 
    ...
    # Create a directory to save files
        path = '/Users/francescoferrini/VScode/MultirelationalGNN/data/simple/deterministic/tot_rel_' + str(args.num_rel_types) + '_metapaths_number_' + str(metapaths_number)
    else: 
        path = '/Users/francescoferrini/VScode/MultirelationalGNN/data/simple/deterministic/tot_rel_' + str(args.num_rel_types) + '_metapaths_number_' + str(metapaths_number)
