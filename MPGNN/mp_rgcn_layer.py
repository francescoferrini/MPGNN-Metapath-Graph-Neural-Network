...
def masked_edge_index(edge_index, edge_mask):
    if isinstance(edge_index, Tensor):
        #print(edge_index)
        # print(edge_mask, len(edge_mask))
        # print(edge_index[:, edge_mask], len(edge_index[:, edge_mask][0]), len(edge_index[:, edge_mask][1]))
        # print('############################################# end')
...
class CustomRGCNConv(MessagePassing):
    ...
    def forward(self, relation, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            if var_bool == True:
                ##### MODIFICATION: We only consider 1 relation for each layer
...
class CustomFastRGCNConv(CustomRGCNConv):
