...
class Net(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, num_rel, output_dim, ll_output_dim, n_layers, metapath_length):
        ...
        self.conv1 = RGCNConv(5000, hidden_dim, num_rel, flow='target_to_source') 
        self.conv2 = RGCNConv(hidden_dim, 2, num_rel, flow='target_to_source')
    ...
    def forward(self, x, edge_index, edge_type):
        ...
        #print(x, F.log_softmax(x, dim=1), x.size(), F.log_softmax(x, dim=1).size())
        return F.log_softmax(x, dim=1)

# class Net(torch.nn.Module):
#     def __init__(self, input_dim, hidden_dim, num_rel, output_dim, ll_output_dim, n_layers):
#         super().__init__()
#         self.n_layers = n_layers
...
#     def forward(self, x, edge_index, edge_type):
#         '''
#         Layer index is both the intex of the layer and also the relation type
#         we are considering in that specific layer
#         '''
#         for layer_index in range(self.n_layers):
#             if layer_index == 0:
#                 x = F.relu(self.conv1(layer_index, x, edge_index, edge_type))
#             else:
#                 x = F.relu(self.conv2(layer_index, x, edge_index, edge_type))
#         x = self.LinearLayer(x)
#         return F.log_softmax(x, dim=1)
